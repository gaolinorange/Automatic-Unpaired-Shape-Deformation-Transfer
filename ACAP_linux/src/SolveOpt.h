...
public:
	...
	virtual double eval_f(const double* _x)
	{
		...
		for (int i = 0; i < num; i++)
			temp(i) = _x[i];
		//Eigen::VectorXd one = Eigen::VectorXd::Ones(1,num);//
	...
	virtual void   eval_gradient(const double* _x, double*    _g)
	{
		...
		Eigen::MatrixXd G = 4 * temp.transpose() * AtplusA - 4 * one * A;//
		for (int i = 0; i < num; i++)
			_g[i] = G(0, i);
	}
...
public:
	...
	{
		//mstream mout;
		//std::streambuf *outbuf = std::cout.rdbuf(&mout);//�ض���cout  
